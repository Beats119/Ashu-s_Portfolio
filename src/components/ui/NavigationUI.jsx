...
export default function NavigationUI(){
 const {currentRoom,setMapOpen,mapOpen,setSoundOn,soundOn,setRecruiter,recruiter,exitRoom}=useScene()
 ...
 return <><header className="nav-ui"><a className="nav-logo" href="#top">AM<span>.</span></a><div className="nav-status">{currentRoom?currentRoom.toUpperCase():'CORRIDOR / '+Math.round(p*100)+'%'}</div><div className="nav-actions"><button onClick={()=>setMapOpen(true)}>MAP</button><button onClick={()=>setSoundOn(!soundOn)}>{soundOn?'SOUND ON':'SOUND OFF'}</button><button onClick={()=>setRecruiter(!recruiter)}>{recruiter?'EXPLORE':'RECRUITER'}</button></div></header><div className="door-trace">{ROOMS.map((r,i)=><span key={r.id} className={(currentRoom===r.id||p>(i+1)/(ROOMS.length+1))?'lit':''}></span>)}</div>{mapOpen&&<MapPanel onClose={()=>setMapOpen(false)}/>} {recruiter&&<RecruiterPanel onClose={()=>setRecruiter(false)}/>} {currentRoom&&<button className="back-ui" onClick={exitRoom}>← BACK TO CORRIDOR</button>}</>
}
function MapPanel({onClose}){
 const {enterRoom}=useScene()
 return <div className="overlay-panel map-panel"><button className="panel-close" onClick={onClose}>ESC / CLOSE</button><div className="map-paper"><div className="panel-kicker">FIELD MAP / NAVIGATION</div><h2>THE WORLD.</h2><div className="map-line"></div><div className="map-rooms">{ROOMS.map((r,i)=><button key={r.id} onClick={()=>{onClose();enterRoom(r.id)}}><span>{String(i+1).padStart(2,'0')}</span><b>{r.label}</b><small>{r.short}</small></button>)}</div></div></div>
}
